...
public class ModbusPipeClientIntegrationTests : IDisposable {
    private MockModbusServer? _server;
    private IModbusClient? _client;
    private bool _disposed;

    public ModbusPipeClientIntegrationTests(ITestOutputHelper output) {
    }

    private MockModbusServer CreateServer() {
        var server = new MockModbusServer();
        server.Start();
        return server;
    }
    ...
    [Fact]
    public async Task ModbusPipeClient_Connect_Test() {
    ...
    [Fact]
    public async Task ModbusPipeClient_WriteSingleCoil_Test() {
        _server = CreateServer();
        _client = CreateClient(_server);
    ...
    [Fact]
    public async Task ModbusPipeClient_ReadCoils_Test() {
        _server = CreateServer();
        _client = CreateClient(_server);
    ...
    [Fact]
    public async Task ModbusPipeClient_WriteSingleRegister_Test() {
        _server = CreateServer();
        _client = CreateClient(_server);
    ...
    [Fact]
    public async Task ModbusPipeClient_ReadHoldingRegisters_Test() {
        _server = CreateServer();
        _client = CreateClient(_server);
    ...
    [Fact]
    public async Task ModbusPipeClient_MultipleOperations_Test() {
        _server = CreateServer();
        _client = CreateClient(_server);
