...
public sealed class ChannelTransport(ChannelSession session) : IModbusTransport {
    ...
    private async Task<byte[]> ReceiveResponseAsync(CancellationToken cancelToken) {
        var response = await session.ServerToClient.Reader.ReadAsync(cancelToken).ConfigureAwait(false);
        if (response.Length == 0) throw new ModbusTimeoutException("Channel 接收超时，未收到响应数据");
        return response;
    }
